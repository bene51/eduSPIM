...
print_format_list(int camIdx)
{
	...
	if(!SAVE_CALL(is_ImageFormat(cam, IMGFRMT_CMD_GET_NUM_ENTRIES, &count, 4), camIdx))
        return;
	...
	if(!SAVE_CALL(is_ImageFormat(cam, IMGFRMT_CMD_GET_LIST, pformatList, bytesNeeded), camIdx)) {
        free(ptr);
        return;
	}
...
camSetup(int camIdx)
{
	...
	if(!SAVE_CALL(is_SetFrameRate(cam, tfps, &rfps), camIdx))
        return;
    printf("Tried setting frame rate to %f, real framerate is %f\n", tfps, rfps);

    double exposure = 0;
	if(!SAVE_CALL(is_Exposure(cam, IS_EXPOSURE_CMD_SET_EXPOSURE, &exposure, 8), camIdx))
        return;
    ...
    for(int i = 0; i < N_BUFFERS; i++) {
        if(!SAVE_CALL(is_AllocImageMem(cam, WIDTH, HEIGHT, bpp, &buffers[camIdx][i], &bid), camIdx))
            return;
        if(!SAVE_CALL(is_AddToSequence(cam, buffers[camIdx][i], bid), camIdx))
            return;
    }
}
...
void camStartSequence(int camIdx)
{
...
void camGetNextSequenceImage(int camIdx, char *image)
{
    int nMemID = 0;
    char *pBuffer = NULL;
    HCAM cam = cameras[camIdx];
...
void camStopSequence(int camIdx)
{
    HCAM cam = cameras[camIdx];
...
main(void) {
    ...
    while(true) {
        printf("Press <Enter> to capture next preview image, type 'quit' to exit.");
        gets(input);
